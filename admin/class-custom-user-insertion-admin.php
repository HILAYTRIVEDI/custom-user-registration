...
if( !class_exists('Custom_User_Insertion_Admin') ){
	...
	class Custom_User_Insertion_Admin {
		...
		public function custom_user_approval_status(){
			...
			if(isset($action) && "urp_verification" === $action){
				if($verified){
					$cst_user = get_users(
						array(
						 'meta_key' => '_user_unique_token_for_admin',
						 'meta_value' => $token,
						)
						);
					$cst_user = $cst_user[0];
					...
					wp_mail($cst_user->user_email,"Approved!!","Congrultaion your registratioon is approved. Now you can login");
					$page = get_page_by_title($userName, OBJECT, 'custom_user');
		 ...
		 */
		public function check_validation_status($user,$username, $password) {
			...
			if(isset($_POST) && !empty($_POST)){
				if ( isset( $_POST['create_user_details'] ) && wp_verify_nonce( $_POST['create_user_details'], 'create_user_details_action' )) {
					...
					if(isset($_POST["custom_user_skills"])):
						update_post_meta($post_id, 'custom_user_skills', $_POST["custom_user_skills"] );
					endif;
